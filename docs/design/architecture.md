...
agent-browser CLI
  ...
  │            browser.launch: rsync 同步 Profile → RemoteDebug 目录，拉起 Chrome，返回 cdpUrl
  │            browser.close : 按 sessionId 终止 Chrome，清理会话
  │
...
## 插件一：agent-browser-plugin-userprofile-browser（browser.provider）

> 该插件负责 Profile 准备与启动。它作为浏览器 provider（`--provider userprofile-browser`）：rsync 同步真实 Profile 到 RemoteDebug 目录后，直接用调试端口拉起 Chrome 并返回 CDP URL，由 agent-browser 接管驱动。

### 职责

`browser.provider` 插件 — 通过 `--provider userprofile-browser` 触发，支持 `browser.launch` / `browser.close` 两种请求类型。`browser.launch` 响应字段为 `browser`，`browser.close` 响应字段为 `data`。

### 核心流程

```
browser.launch（--provider userprofile-browser）
  │
  ├─ 若提供 request.cdpUrl：connect 模式，直接返回该 CDP URL（不启动）
  ...
  │     并写入状态文件 { userDataDir, profileDirectory, source, syncedAt }
  ├─ 解析 Chrome 可执行文件，spawn（detached）启动：
  │     --remote-debugging-port=<port> --user-data-dir=<debug> --profile-directory=<profile>
  │     --no-first-run --no-default-browser-check --disable-blink-features=AutomationControlled
  ├─ 轮询 <debug>/DevToolsActivePort 得到端口与 browser ws 路径，拼出 cdpUrl
  ├─ 在会话注册表中登记 { sessionId, pid, port, cdpUrl, ... }
  └─ 返回 { browser: { cdpUrl, directPage:false, metadata, cleanup:{ sessionId } } }

browser.close（request.sessionId）
  ├─ 从会话注册表查 sessionId；找不到 → 幂等 { data: { closed:false, noOp:true } }
  ├─ launch 模式：process.kill(pid) 终止该 Chrome
  └─ 返回 { data: { closed, removedDebugDir } }（removeDebugDir:true 时一并删除 debug 目录）
```
...
Chrome 可执行文件:
  ...
  2. 平台默认（macOS: /Applications/Google Chrome.app/...；Linux: PATH 上的 google-chrome/chromium）

会话注册表（供 browser.close 终止进程）:
  <cwd>/.agent-browser/userprofile-browser-sessions.json
    { sessionId: { mode, pid, port, cdpUrl, userDataDir, profileDirectory, startedAt } }
```

### 关键设计决策

- **rsync 只执行一次**：状态文件存在则跳过，`force:true` 可重新同步。
- **绕过 “non-default data directory” 限制**：全量 rsync 让 RemoteDebug 目录看起来像真实 Profile。
- **只对同步副本启停 Chrome**：provider 只启动 RemoteDebug 副本（非真实 Profile 目录），并通过会话注册表按 sessionId 终止；从不删除真实 Chrome 的 Profile 锁。rsync 排除锁/日志/journal/cache，即使真实 Chrome 运行中也能安全拷贝。
...
│   │   ├── package.json
│   │   └── tsconfig.json
│   └── agent-browser-plugin-userprofile-browser/  # browser.provider: Profile rsync + 拉起 Chrome
│       ├── src/index.ts
│       ├── dist/index.js
│       ├── build.ts
│       ├── package.json
